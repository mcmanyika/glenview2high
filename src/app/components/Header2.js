  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const schoolRef = ref(database, 'account'); // Reference to 'account' collection
        onValue(schoolRef, (snapshot) => {
          const schoolData = snapshot.val();
          if (schoolData) {
            // Assuming there is only one account entry
            const accountKeys = Object.keys(schoolData);
            if (accountKeys.length > 0) {
              setSchoolName(schoolData.schoolName); // Set school name
            }
          }
        });
      } catch (error) {
    ...
    <header className="fixed top-0 z-50 w-full bg-blue-400 text-white p-4 transition-all duration-500 ease-in-out">
      ...
      <nav className="max-w-5xl mx-auto flex justify-between items-center">
        <div className="flex items-center w-4/5 space-x-2">
          ...
          <h1 className={`text-sm md:text-2xl font-normal uppercase ${isOpen ? 'hidden' : 'block'}`}>{schoolName}</h1>
        </div>
