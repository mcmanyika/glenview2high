  ...
  return (
    <div>
      ...
      <table className="min-w-full text-sm text-left border-collapse border border-gray-200">
        ...
        <tbody>
          {currentAdmissions.map((admission) => (
            <tr key={admission.id}>
              <td className="border border-gray-200 px-4 py-2">{admission.admissionId}</td>
      ...
      {paymentModalOpen && selectedAdmission && (
        <PaymentModal
          studentId={selectedAdmission.admissionId} // Pass the admissionId as studentId to PaymentModal
          onClose={() => {
